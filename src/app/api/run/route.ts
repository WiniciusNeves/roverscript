...
export async function POST(request: Request) {
  try {
    ...
    const { code, initialObstacles } = body;
    ...
    if (initialObstacles && Array.isArray(initialObstacles)) {
      initialObstacles.forEach((obs: string) => {
      ...
      for (const command of commandStream) {
        if (command.type === "Move") {
          if (command.payload === command.requested) {
            executionLogs.push(`[AÇÃO] Movendo ${command.payload} passos com sucesso.`);
          } else {
            executionLogs.push(`[ALERTA] Colisão/Borda detectada. Dos ${command.requested} passos, moveu apenas ${command.payload}.`);
          }
        } else if (command.type === "Turn") {
          executionLogs.push(`[AÇÃO] Virando para a ${command.payload}.`);
        } else if (command.type === "PlaceObstacle") {
          executionLogs.push(`[SISTEMA] Obstáculo criado em (${command.payload.x}, ${command.payload.y}).`);
        }
      }
    }
